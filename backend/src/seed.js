...
const l2Stations = [
  { name: 'Tham Lương', code: 'L2-20', location: { lat: 10.8227, lng: 106.6200 }, facilities: ['Toilet', 'Elevator'], status: 'maintenance' },
  { name: 'Tân Bình', code: 'L2-21', location: { lat: 10.8100, lng: 106.6300 }, facilities: ['ATM'], status: 'maintenance' }
];

async function seed() {
  try {
    const mongoUri = process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/node_auth_db';
    ...
    const dbL1 = await Line.create(line1Data);
    const dbL2 = await Line.create(line2Data);
    ...
    console.log('Successfully seeded metro data');
    process.exit(0);
  } catch (err) {
